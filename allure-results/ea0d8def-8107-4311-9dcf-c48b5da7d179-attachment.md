...
Call log:
  - waiting for locator('a').filter({ hasText: 'Main Dispensarydispensary' })
...
> 23 |   await this.page.locator('a').filter({ hasText: 'Main Dispensarydispensary' }).click();
     |                                                                                 ^ Error: locator.click: Test timeout of 30000ms exceeded.
  24 |   await expect(this.page.locator('h6')).toContainText('You are currently in Main Dispensary dispensary. To change, you can always click here.');
  25 |   await this.page.locator('div').filter({ hasText: /^Night Counter click to Activate$/ }).nth(2).click();
  26 |   
  27 |   }
  28 | }
```